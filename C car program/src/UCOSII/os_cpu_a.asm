...
OS_CPU_SR_Save
    MRS     R0, PRIMASK                                         ; 读特殊功能寄存器（屏蔽寄存器）到通用寄存器Set prio int mask to mask all (except faults)
    CPSID   I													;关中断
    BX      LR													;返回程序

OS_CPU_SR_Restore												
    MSR     PRIMASK, R0											;存储通用寄存器的值到特殊寄存器中
    BX      LR													;返回
...
OSStartHighRdy
    ...
    STRB    R1, [R0]											;把NVIC_PENDSV_PRI优先级设置为14存储寄存器中的低字节

    MOVS    R0, #0                                              ; Set the PSP to 0 for initial context switch call
    MSR     PSP, R0												;初始化进程堆栈，存储通用寄存器的值到特殊寄存器中
